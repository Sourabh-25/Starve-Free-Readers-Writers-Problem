...
class semaphore {    // we accomodate binary and counting semaphores in same class.
     
    
    public:
    long long value; 
        
    semaphore(int n){     //here n is the number of resources of that type available
      value=n;
    }

void signal() { //standard V(S) method
    value++; // value of semaphore is incremented as signal is called.
return;
}

void wait() { // standard P(S) method
    while(value<=0){
...
void starveFreeReader(int PID) { // PID is process id of the reader
    ...
    if(rdCnt == 1) {   // it would be the first reader
